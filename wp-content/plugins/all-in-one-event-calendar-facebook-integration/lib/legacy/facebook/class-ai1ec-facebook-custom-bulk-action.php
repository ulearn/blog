...
class Ai1ec_Facebook_Custom_Bulk_Action extends Ai1ec_Base {
	...
	public function facebook_filter_restrict_manage_posts() {
		...
		if( $typenow === AI1EC_POST_TYPE ) {
			...
			$all_evts_lbl    = __( "Show all events", AI1ECFI_PLUGIN_NAME );
			$imported_lbl    = __( "Show only events imported from Facebook", AI1ECFI_PLUGIN_NAME );
			$exported_lbl    = __( "Show only events exported to Facebook", AI1ECFI_PLUGIN_NAME );
			$exportable_lbl  = __( "Show only events that can be exported to Facebook", AI1ECFI_PLUGIN_NAME );
	...
	public function facebook_custom_bulk_action() {
		...
		if( ( ( isset( $_GET['action'] ) && $_GET['action'] === 'export-facebook' ) || ( isset( $_GET['action2'] ) && $_GET['action2'] === 'export-facebook' ) ) && isset( $_GET['post'] ) ) {
			...
			if( $this->facebook_plugin->check_if_we_have_a_valid_access_token() ) {
				...
				$message = array(
						"label"        => __( 'All-in-One Event Calendar Facebook Export Error', AI1ECFI_PLUGIN_NAME ),
						"message"      => __( "We couldn't export data to Facebook as we don't have a valid access token. Try to log out of Facebook from the <strong>Events</strong> &gt; <strong>Calendar Feeds</strong> screen, then log in again.", AI1ECFI_PLUGIN_NAME ),
						"message_type" => "error",
				);
