...
class Ai1ec_Facebook_Graph_Object_Collection extends Ai1ec_Base {
	...
	static public $fb_all_types = array( self::FB_PAGE, self::FB_GROUP );
	/**
	...
	static public function get_type_printable_text( $type ) {
		$text = '';
		switch ( $type ) {
			case self::FB_USER: $text = esc_html__( 'Friends', AI1ECFI_PLUGIN_NAME );
				break;
			case self::FB_PAGE: $text = esc_html__( 'Pages', AI1ECFI_PLUGIN_NAME );
				break;
			case self::FB_GROUP: $text = esc_html__( 'Groups', AI1ECFI_PLUGIN_NAME );
				break;
		}
		return $text;
	}
	...
	public function update_subscription_status( $subscribe = TRUE, $remove_events = FALSE ) {
		...
		foreach ( $data as $id ) {
			...
			if ( $result === FALSE ) {
				...
				$error_messages[] = sprintf( __( $message, AI1ECFI_PLUGIN_NAME ), $fgo->get_user_name() );
	...
	private function update_facebook_users_db( array $users ) {
		...
		foreach ( $users as $id => $user ) {
			...
			if( $result === FALSE ) {
				...
				$error_messages[] = sprintf( __( "An error message occurred when saving %s data to the db", AI1ECFI_PLUGIN_NAME ), $user['user_name'] );
			}
		}
		// If there are error messages, throw the appropriate Exception.
		if( ! empty( $error_messages ) ) {
	...
	private function save_event( array $event, $user_timezone ) {
		...
		if ( $new_post_id === 0 ) {
			throw new Exception(
				__( 'Could not save a new event during Facebook import.', AI1ECFI_PLUGIN_NAME )
			);
		}
	}
	/**
	...
	private function convert_facebook_event_to_ai1ec_event( array $facebook_event, $user_timezone, $post_id = NULL ) {
		...
		$event->set( 'timezone_name', get_option( 'timezone_string' ) );
		...
		if (
			...
			/**
			 * @TODO: adjust when model.event.entity::_end defaults to 0
			 */
		...
		if ( $facebook_event['has_profile_pic'] ) {
			$img = isset( $facebook_event['pic_cover']['source'] )
				? $facebook_event['pic_cover']['source']
				: $facebook_event['pic_big'];
			list( , , , $attr ) = getimagesize( $img );
			$post['post_content'] =
	...
	private function update_event( array $event, $user_timezone, $post_id ) {
		...
		if ( $return === 0 ) {
			throw new Exception(
				__( 'Could not update an event post during Facebook import.', AI1ECFI_PLUGIN_NAME )
			);
		}
