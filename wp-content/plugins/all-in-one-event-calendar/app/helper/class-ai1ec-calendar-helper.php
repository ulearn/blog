...
class Ai1ec_Calendar_Helper {
	 ...
	 *                                ['auth_ids']  => non-associatative array of event author IDs
	...
	function _get_filter_sql( &$filter ) {
		...
		foreach( $filter as $filter_type => $filter_ids ) {
			// If no filter elements specified, do not do anything
			if( $filter_ids && is_array( $filter_ids ) ) {
				switch ( $filter_type ) {
					...
					// Limit by author IDs
					case 'auth_ids':
						$filter['filter_where']  .= $where_logic . " p.post_author IN ( " . join( ',', $filter_ids ) . " ) ";
						$where_logic = $inner_logic;
						break;
				}
			}
	...
	public function get_html_for_authors( $view_args ) {
		...
		$href_for_clearing_filter = $this
			->generate_href_without_arguments(
				$view_args,
				array( 'auth_ids' )
			);
		...
		$args = array(
			'authors'           => $authors,
			'selected_auth_ids' => $view_args['auth_ids'],
			'data_type'         => $view_args['data_type'],
			'clear_filter'      => $href_for_clearing_filter,
		);
	...
	/**
	 * Check if the buy ticket button is enabled for the given view.
	 *
	 * @param string $view
	 * @return boolean
	 */
