...
class Ai1ec_Event {
	...
	protected function _handle_property_construct_cost( $value ) {
		...
		$this->is_free = (bool)$is_free;
		return true;
	}

	/**
	 * _handle_property_destruct_cost method
	 *
	 * Handle `cost` writing to permanent storage.
	 *
	 * @return string Serialized value to store
	 */
	protected function _handle_property_destruct_cost() {
		$cost = array(
			'cost'    => $this->cost,
			'is_free' => false,
		);
	...
	/**
	 * _handle_property_construct method
	 *
	...
	/**
	 * _handle_property_destruct method
	 *
	 ...
	 */
	public function get_tickets_url_label( $long = true ) {
		if ( $this->is_free ) {
			return ( $long )
