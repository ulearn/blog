...
<ul class="nav nav-tabs">
	...
	<li class="dropdown">
		...
		<ul class="dropdown-menu">
			...
			<li><a href="#ai1ec-apis" data-toggle="tab"><?php _e( 'External Services', AI1EC_PLUGIN_NAME ); ?></a></li>
		</ul>
	</li>
...
<div class="tab-content ai1ec-boxed-tabs">
	...
	<div class="tab-pane" id="ai1ec-advanced">
		...
		<label for="enable_user_event_notifications">
			<input class="checkbox" name="enable_user_event_notifications" id="enable_user_event_notifications" type="checkbox" value="1" <?php echo $enable_user_event_notifications; ?> />
			<?php _e( 'Allow users to request <strong>email notifications</strong> for upcoming events', AI1EC_PLUGIN_NAME ) ?>
		</label>

		<div>
			<label for="disable_standard_filter_menu">
				<input name="disable_standard_filter_menu"
					id="disable_standard_filter_menu"
					type="checkbox" class="checkbox" value="1"
					<?php echo $disable_standard_filter_menu; ?>
					/>
				<?php printf(
					__(
						'<strong>Hide standard filter menu</strong>; rely on theme to insert filter menu <span %s>using this template tag</span>',
						AI1EC_PLUGIN_NAME
					),
					'data-toggle="ai1ec_collapse" data-target="#disable_standard_filter_menu_sample" class="ai1ec-tooltip-toggle" id="disable_standard_filter_menu_toggler"'
					); ?>
			</label>
			<div class="clear"></div>
			<div class="ai1ec-row collapse"
			     id="disable_standard_filter_menu_sample">
		...
		<?php endif; ?>
			...
			<label for="require_disclaimer">
				<input class="checkbox" name="require_disclaimer" id="require_disclaimer" type="checkbox" value="1"
					<?php echo $require_disclaimer; ?> />
				<?php _e( 'Require users to agree to a <strong>disclaimer</strong> before posting an event from the front-end form', AI1EC_PLUGIN_NAME ); ?>
				<span class="help-block"><?php _e( 'If you enable this, you must provide the disclaimer text (HTML allowed) in the field below', AI1EC_PLUGIN_NAME ); ?></span>
			</label>
			<div class="row-fluid">
				<label for="disclaimer">
					<?php _e( 'Disclaimer:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<textarea name="disclaimer" id="disclaimer" class="span12"
					rows="4"><?php echo esc_html( $disclaimer ); ?></textarea>
			</div>
		<div class="clear"></div>
		<h4><?php _e( 'Permalinks', AI1EC_PLUGIN_NAME ); ?></h4>
			<label for="calendar_base_url_for_permalinks">
				<input class="checkbox" name="calendar_base_url_for_permalinks"
				id="calendar_base_url_for_permalinks" type="checkbox" value="1"
					<?php echo $calendar_base_url_for_permalinks; ?> />
				<?php
				_e(
	...
	<div class="tab-pane" id="ai1ec-apis">
		<h2><?php _e( 'External Services', AI1EC_PLUGIN_NAME ); ?></h2>

		<?php do_action( 'ai1ec_api_settings_before' ); ?>

		<fieldset class="clear ai1ec-oauth-twitter-settings">
			<legend>
				<?php _e( 'Twitter connection settings', AI1EC_PLUGIN_NAME ); ?>
			</legend>

			<div class="row-fluid">
				<label for="oauth_twitter_id">
					<?php _e( 'Application <strong>Consumer Key</strong>:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<input name="oauth_twitter_id" id="oauth_twitter_id" type="text"
					class="span12"
					value="<?php echo esc_attr( $oauth_twitter_id ); ?>" />
			</div>

			<div class="row-fluid">
				<label for="oauth_twitter_pass">
					<?php _e( 'Application <strong>Consumer Secret</strong>:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<input name="oauth_twitter_pass" id="oauth_twitter_pass" type="text"
					class="span12"
					value="<?php echo esc_attr( $oauth_twitter_pass ); ?>" />
			</div>

			<span class="help-block"><?php
				global $ai1ec_oauth_controller;
				$oauth_twitter_authorize_url = $ai1ec_oauth_controller
					->get_callback_url( 'twitter' );
				printf(
					__(
						'Use "<em>%s</em>" URL for <strong>Callback URL</strong> when configuring your <a href="https://dev.twitter.com/apps/new">Twitter application</a>. After creating the application, change the permissions required to <strong>Read and Write</strong> on the <strong>Settings</strong> tab in Twitter.',
						AI1EC_PLUGIN_NAME
					),
					$oauth_twitter_authorize_url
				);
			?></span>

			<?php if ( ! empty( $oauth_twitter_id ) ) : ?>
			<span class="help-block"><?php
				printf(
					__(
			...
			<div class="clearfix">
				<label class="textinput" for="twitter_notice_interval">
					<?php _e( 'Time to notification before event start:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<input name="twitter_notice_interval" id="twitter_notice_interval" type="text"
					class="textinput input-small"
					value="<?php echo esc_attr( $twitter_notice_interval ); ?>" />
				<span class="help-block"><?php _e( 'Announcements will be posted to Twitter this long before start of event. Enter time in seconds (default behavior), or use suffixes, for example: <strong>3h</strong> = <em>3 hours</em>; <strong>1d</strong> = <em>1 day</em>.', AI1EC_PLUGIN_NAME ) ?></span>
			</div>

		</fieldset>


		<?php do_action( 'ai1ec_api_settings_after' ); ?>
	</div>
	...
	<div class="tab-pane" id="ai1ec-mail">
		...
		<fieldset>
			<legend>
				<?php _e( 'Mail sent to admin when new event is submitted from the front-end form:', AI1EC_PLUGIN_NAME ); ?>
			</legend>
			<div class="row-fluid">
				<label for="admin_add_new_event_mail_subject">
					<?php _e( 'Subject:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<input name="admin_add_new_event_mail_subject" id="admin_add_new_event_mail_subject" type="text"
					class="span12"
					value="<?php echo esc_attr( $admin_add_new_event_mail_subject ); ?>" />
			</div>
			<div class="row-fluid">
				<label for="admin_add_new_event_mail_body">
					<?php _e( 'Body:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<textarea name="admin_add_new_event_mail_body" id="admin_add_new_event_mail_body" class="span12"
					rows="4"><?php echo esc_html( $admin_add_new_event_mail_body ); ?></textarea>
			</div>
		</fieldset>
		<fieldset>
			<legend>
				<?php _e( 'Upcoming event notifications sent to subscribed users:', AI1EC_PLUGIN_NAME ); ?>
			</legend>
			<div class="row-fluid">
				<label for="user_upcoming_event_mail_subject">
					<?php _e( 'Subject:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<input name="user_upcoming_event_mail_subject" id="user_upcoming_event_mail_subject" type="text"
					class="span12"
					value="<?php echo esc_attr( $user_upcoming_event_mail_subject ); ?>" />
			</div>
			<div class="row-fluid">
				<label for="user_upcoming_event_mail_body">
					<?php _e( 'Body:', AI1EC_PLUGIN_NAME ); ?>
				</label>
				<textarea name="user_upcoming_event_mail_body" id="user_upcoming_event_mail_body" class="span12"
					rows="4"><?php echo esc_html( $user_upcoming_event_mail_body ); ?></textarea>
			</div>
		</fieldset>
