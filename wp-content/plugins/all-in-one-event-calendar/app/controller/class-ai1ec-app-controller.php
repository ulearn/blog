...
class Ai1ec_App_Controller {
	...
	private function __construct( $preview_mode )
	{
		...
		$aie1c_deferred_rendering_helper =
			Ai1ec_Deferred_Rendering_Helper::get_instance();
		$ai1ec_duplicate_controller
			->set_admin_notice_helper( $aie1c_deferred_rendering_helper );
		$css_controller = Ai1ec_Less_Factory::create_css_controller_instance();
		$notification_controller =
			Ai1ec_Notification_Controller::get_instance();
		...
		add_action( 
			Ai1ec_Deferred_Rendering_Helper::ADMIN_MESSAGES_ACTION, 
			array( $aie1c_deferred_rendering_helper, 'render' )
		);
		// Render modals in the footer
		add_action( 
			Ai1ec_Deferred_Rendering_Helper::BOOTSTRAP_MODALS_ACTION, 
			array( $aie1c_deferred_rendering_helper, 'render' )
		);
		...
		// subscribe to event
		add_action( 'wp_ajax_nopriv_ai1ec_subscribe_to_event',
			array( $notification_controller, 'subscribe_to_event' )
		);
		add_action( 'wp_ajax_ai1ec_subscribe_to_event',
			array( $notification_controller, 'subscribe_to_event' )
		);
		...
		add_action(
			...
			array( $notification_controller, 'send_twitter_messages' )
		);
	...
	/**
	 * Process request and call appropriate controller directly
	 *
	 * Method checks for `plugin`, `controller` and `action` arguments in
	 * query (either POST, or GET) and if these are present, match some
	 * controller in plug-in stack and action (method) is callable - pass
	 * call to it.
	 *
	 * @return mixed If method is not found - bool(false), output of the
	 *               method otherwise
	 */
	public function parse_standalone_request() {
		...
		foreach ( $shortcuts as $key => $parameters ) {
			if ( isset( $_REQUEST[$key] ) ) {
				extract( $parameters['options'], EXTR_IF_EXISTS );
			}
		}

		// safeguard list, to protect from calling arbitrary methods
		$safe_list   = array(
			'ai1ec_exporter_controller' => array(
				'export_events'  => true,
			),
			'ai1ec_oauth_controller'    => array(
				'handle_request' => true,
			),
		);
		// list of controllers, that don't require plug-in name to be
		// present for brevity
		$skip_plugin = array(
			'ai1ec_oauth_controller'    => true,
		);

		$plugin     = $ai1ec_app_helper->get_param( 'plugin', $plugin );
		$controller = $ai1ec_app_helper->get_param( 'controller', $controller );
		$action     = $ai1ec_app_helper->get_param( 'action', $action );

		if (
			(string)AI1EC_PLUGIN_NAME !== (string)$plugin &&
			! isset( $skip_plugin[$controller] ) ||
			! isset( $safe_list[$controller] ) ||
			! isset( $safe_list[$controller][$action] )
		) {
			return false;
		}
		...
		if ( isset( $GLOBALS[$controller] ) ) {
			$object = $GLOBALS[$controller];
		} else {
			$initializer = Ai1ec_Inflector_Utility::classify( $controller ) .
				'::get_instance';
		...
		if (
			NULL === $object ||
			! is_callable( array( $object, $action ) )
		) {
			return false;
		}
