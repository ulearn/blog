...
class Ai1ec_Loader
{
	...
	/**
	 * Return list of classes matching supplied regular expression
	 *
	 * @param string $regexp Regular expression to match class name against
	 *
	 * @return array List of matching classes
	 */
	public function get_matches( $regexp ) {
		$output = array();
		foreach ( $this->_paths as $class => $files ) {
			if ( 1 === preg_match( $regexp, $class ) ) {
				$output[] = $class;
			}
		}
		return $output;
